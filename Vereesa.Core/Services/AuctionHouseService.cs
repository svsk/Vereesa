    ...
    public class AuctionHouseService : BotServiceBase
    {
        ...
        private async Task HandleMessageReceivedAsync(SocketMessage message)
        {
            if (message?.GetCommand() == "!ah")
            {
                ...
                if (itemStats.item != null) 
                {
                    ...
                    builder.AddField("__Current Quantity__", item.Quantity.ToString("#,0", StringExtensions.GetThousandSeparatorFormat()), true);
                    
                    var auctions = itemStats.item.Auctions.AuctionList;
                    var topAuctions = auctions.OrderBy(auc => auc.BuyoutPrice / auc.Quantity).Take(5).Select(auc => {
        ...
        private class UndermineItem 
        {
            ...
            { 
                get 
                {
                    return (Price / 10000).ToString("#,0.00", StringExtensions.GetThousandSeparatorFormat());
                }
            } 
        }
