...
namespace Vereesa.Core.Services
{
	public class CoronaService : BotServiceBase
	{
		...
		private Statistics _coronaStats => _statRepository.FindById("corona") ??
			new Statistics { Id = "corona" };
		private Statistics _flags => _statRepository.FindById("flags") ??
			new Statistics { Id = "flags" };

		public CoronaService(DiscordSocketClient discord, IRepository<Statistics> statRepository,
			ILogger<CoronaService> logger)
			: base(discord)
		{
			_discord = discord;
		...
		public async Task HandleCoronaCheckAsync(IMessage message)
		{
			...
			var result = GetCoronaReport(handlers =>
				handlers.OnStepCompleted = async (progress) => await messageToUpdate
					.ModifyAsync(msg => msg.Content = progress.ToString())
			);
