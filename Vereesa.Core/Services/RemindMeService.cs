...
namespace Vereesa.Core.Services
{
	public class RemindMeService : BotServiceBase
	{
		...
		public async Task DeleteReminder(IMessage message)
		{
			...
			var reminderSelction = $"Which reminder would you like me to delete?\n" +
				string.Join("\n", reminders.Select((item, index) =>
					$"[{index + 1}] {item.Message}"
				)) + "\n" +
				"Select by responding with the number corresponding to the item you want to remove. Like this `1`.";

			var response = await Prompt(message.Author, reminderSelction, message.Channel, 30000);
		...
		{
			var abort = new Func<string, Task>(async (reason) =>
			{
				await message.Channel.SendMessageAsync($"❌ {reason} Please try again.");
			});

			var prompt = new Func<string, int, Task<IMessage>>(async (promptMessage, timeout) =>
			{
				return await Prompt(message.Author, $"🎗 {promptMessage}", message.Channel, timeout);
			});
			...
			if (channel == null || !ulong.TryParse(channel.Content.Replace("<#", "").Replace(">", "").Trim(), out var channelId))
			{
