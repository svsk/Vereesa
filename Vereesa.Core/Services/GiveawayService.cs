...
namespace Vereesa.Core.Services
{
	public class GiveawayService : BotServiceBase
	{
		...
		private Func<string, string> _durationPromptMessage =
			(giveawayChannel) => $":tada: Sweet! The giveaway will be in {giveawayChannel}! Next, how long should the giveaway last?\r\n\r\n`Please enter the duration of the giveaway in seconds.\r\nAlternatively, enter a duration in minutes and include an M at the end.`";

		private Func<long, string, string> _winnerCountPromptMessage =
			(duration, timeUnit) => $":tada: Neat! This giveaway will last **{duration}** {timeUnit}! Now, how many winners should there be?\r\n\r\n`Please enter a number of winners between 1 and 15.`";

		private Func<int, string> _prizePromptMessage =
			(numberOfWinners) => $":tada: Ok! {numberOfWinners} winners it is! Finally, what do you want to give away?\r\n\r\n`Please enter the giveaway prize. This will also begin the giveaway.`";
		...
		private async Task ConfigureGiveaway(IMessageChannel configChannel, IUser author)
		{
			...
			var channel = await PromptUntilReceived(cancellationTokenSource, _channelPromptMessage(), author, configChannel, ParseChannel);

			var duration = await PromptUntilReceived(cancellationTokenSource, _durationPromptMessage(channel.MentionChannel()), author, configChannel, ParseDuration);

			var winnerCount = await PromptUntilReceived(cancellationTokenSource, _winnerCountPromptMessage(duration.duration, duration.timeUnit), author, configChannel, ParseWinnerCount);

			var prize = await PromptUntilReceived(cancellationTokenSource, _prizePromptMessage(winnerCount), author, configChannel, (messageContent) => messageContent);

			if (cancellationTokenSource.IsCancellationRequested)
			{
				// cancel
				await configChannel.SendMessageAsync(":boom: Alright, I guess we're not having a giveaway after all...\r\n\r\n`Giveaway creation has been cancelled.`");
			}
			else
			{
				// finalize and do!
				...
				await configChannel.SendMessageAsync($":tada: Done! The giveaway for `{prize}` is starting in {channel.MentionChannel()}!");
			}
		}

		private int CalculateDuration((int duration, string timeUnit) durationTuple) =>
			durationTuple.timeUnit == "minutes" ?
				durationTuple.duration * 60 :
		...
		private int ParseWinnerCount(string messageContent)
		{
			...
			{
				throw new PromptParseValidationException("Please put in a single numeric value for number of winners (between 1 and 15).");
			}
		...
		)
		{
			...
			while (!cancellationTokenSource.IsCancellationRequested && !parseCompleted)
			{
				...
				problems.Clear();
				var promptResponse = await Prompt(author, currentPromptMessage, configChannel, 60000);
		...
		private (int duration, string timeUnit) ParseDuration(string messageContent)
		{
			...
			{
				throw new PromptParseValidationException("Please input a numeric value as duration in seconds. You can also add an 'm' at the end if you want to give it in minutes instead.");
			}
		...
		private ulong ParseChannel(string messageContent)
		{
			...
			var targetChannel = channelId != null ? Discord.GetChannel(channelId.Value) : null;
		...
		private async Task FinalizeAndAnnounceNewGiveaway(Giveaway giveaway)
		{
			...
			var channel = Discord.GetChannel(giveaway.TargetChannel.ToChannelId().Value) as ISocketMessageChannel;
		...
		{
			return Discord.Guilds.Where(g => g.Channels.Any(c => c.Id == channelId)).FirstOrDefault().Channels.First(c => c.Id == channelId) as ISocketMessageChannel;
		}
	...
	internal class PromptParseValidationException : Exception
	{
		...
		public PromptParseValidationException(string message) : base(message)
		{
		}

		public PromptParseValidationException(string message, Exception innerException) : base(message, innerException)
		{
		}

		protected PromptParseValidationException(SerializationInfo info, StreamingContext context) : base(info, context)
		{
		}
	}
}