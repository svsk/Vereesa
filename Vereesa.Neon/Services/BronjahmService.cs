...
namespace Vereesa.Neon.Services
{
    public class BronjahmService : IBotService
    {
        ...
        private ulong _bagBoiRole = 776483894269575189;
        private ulong _channelId => 124246560178438145;
        private static readonly Instant _eventEndTime = Instant.FromUnixTimeMilliseconds(
            1606172400000 - (60 * 60 * 1000)
        );
        ...
        public BronjahmService(IMessagingClient messaging, IJobScheduler scheduler)
        {
        ...
        private IMessageChannel Channel => _messaging.GetChannelById(_channelId) as IMessageChannel;
        ...
        public async Task ShowNextSpawn(IMessage message)
        {
            ...
            if (nextSpawnInstant > _eventEndTime)
            {
                await message.Channel.SendMessageAsync(
                    ":handbag: No more bag boi spawns to come. Enjoy the new expansion!"
                );
                return;
            }
            ...
            var response =
                $":handbag: Bag boi will spawn at **{nextSpawn.ToPrettyTime()} server time**."
        ...
        public async Task ShowNextRare(IMessage request)
        {
            var now = Now();
            var pointInCycle =
                ((now - _bronjahmBaselineSpawnTime).TotalSeconds - _activationTimeInSeconds) % _cycleDurationInSeconds;
        ...
        private void ScheduleNextNotifications()
        {
            ...
            if (now < warningTime)
            {
                ...
                _scheduler.Schedule(
                    ...
                    async () =>
                    {
                        await Channel.SendMessageAsync(warningMessage);
                    }
                );
            }
