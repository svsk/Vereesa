...
public class EventHostService : IBotModule
{
    ...
    [OnInterval(Minutes = 2)]
    public Task UpdateEventRoles()
    {
        _ = EnsureEventRolesUpdated();
        return Task.CompletedTask;
    }

    private async Task EnsureEventRolesUpdated()
    {
        var events = await _events.GetGuildEvents(WellknownGuilds.Neon);
        foreach (var guildEvent in events)
        {
            var participantIds = await _events.GetGuildEventParticipants(guildEvent.GuildId, guildEvent.Id);
            var roles = _messaging.GetRolesByName(guildEvent.Name);
            if (!roles.Any())
            {
                var role = await _messaging.CreateRole(guildEvent.GuildId, guildEvent.Name);
                roles.Add(role);
            }

            foreach (var role in roles)
            {
                var roleMembers = await _messaging.GetRoleUsers(role.Id);
                var roleMemberIds = roleMembers.Select(u => u.Id);

                var memberIdsToRemove = roleMemberIds.Except(participantIds);
                var memberIdsToAdd = participantIds.Except(roleMemberIds);

                var addRoleTasks = memberIdsToAdd.Select(
                    userId => _messaging.AddRoleAsync(role.Guild.Id, role.Id, userId)
                );

                var removeRoleTasks = memberIdsToRemove.Select(
                    userId => _messaging.RemoveRoleAsync(role.Guild.Id, role.Id, userId)
                );

                await Task.WhenAll(addRoleTasks.Concat(removeRoleTasks));
            }
        }
    }
    ...
    public async Task ScheduleRaids(IMessage message, string numberOfWeeks, string weekDays, string raidStart)
    {
        ...
        var daysArray = await ai.QueryAs<string[]>(
            "Give me the weekdays mentioned in this text as a JSON array with full day names in lower case: " + weekDays
        );
