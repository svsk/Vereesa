...
namespace Vereesa.Neon.Services
{
    public class ChannelRuleService : IBotService
    {
        ...
        public ChannelRuleService(DiscordSocketClient discord, IJobScheduler scheduler, ChannelRuleSettings config)
        {
            _discord = discord;
        ...
        private async Task InitializeServiceAsync()
        {
            ...
            ruleset.AddRulesBrokenReaction(
                new RuleReaction<string>(
                    ChannelRuleReactions.AnnounceChannelRulesAsync,
        ...
        {
            var channelRulesets = _rulesets
                .Where(rs => rs.Triggers.Contains(RulesetTriggers.Periodic))
            ...
            foreach (var channelRuleset in channelRulesets)
            {
                try
                {
                    ...
                    var messages = await messageChannel.GetMessagesAsync(20).Flatten().ToListAsync();
        ...
        public async Task MessageReceivedHandler(IMessage receivedMessage)
        {
            var onMessageRulesets = _rulesets
                .Where(
                    rs => rs.ChannelId == receivedMessage.Channel.Id && rs.Triggers.Contains(RulesetTriggers.OnMessage)
                )
                .ToList();
    ...
    public class ChannelRuleEvaluators
    {
        ...
        public static bool MessageMustContainImage(IMessage message)
        {
            ...
            if (
                Regex.IsMatch(message.Content, imagePattern)
                || message.Attachments.Any(att => Regex.IsMatch(att.Url, imagePattern))
            )
            {
                return true;
            }
        ...
        public static bool MessageMustContainYoutubeLink(IMessage message)
        {
            ...
            if (
                Regex.IsMatch(message.Content, youtubePattern)
                || message.Attachments.Any(att => Regex.IsMatch(att.Url, youtubePattern))
            )
            {
                return true;
            }
        ...
        public static bool MessageMustContainTwitchLink(IMessage message)
        {
            ...
            if (
                Regex.IsMatch(message.Content, twitchPattern)
                || message.Attachments.Any(att => Regex.IsMatch(att.Url, twitchPattern))
            )
            {
                return true;
            }
    ...
    public class ChannelRuleReactions
    {
        ...
        public static async Task AnnounceChannelRulesAsync(IMessage message, string channelRules)
        {
            ...
            var previousMessages = await message.Channel.GetMessagesAsync(5).Flatten().ToListAsync();
