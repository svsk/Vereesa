...
namespace Vereesa.Neon.Services
{
    public class CoronaService : IBotService
    {
        ...
        public CoronaService(IRepository<Statistics> statRepository, ILogger<CoronaService> logger)
        {
        ...
        public async Task HandleCoronaCheckAsync(IMessage message)
        {
            ...
            var result = GetCoronaReport(
                handlers =>
                    handlers.OnStepCompleted = async (progress) =>
                        await messageToUpdate.ModifyAsync(msg => msg.Content = progress.ToString())
            );
        ...
        private string GenerateRelevantCountryReport(ProgressHandlers progressHandlers)
        {
            ...
            {
                progressHandlers.OnStepCompleted?.Invoke(
                    $"`⌛ Checking {country.index + 1}/{distinctCountries.Count}: {country.Name}...`"
                );
