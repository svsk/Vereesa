...
namespace Vereesa.Core.Tests.ServicesTests
{
	public class RemindMeServiceTests
	{
		...
		public async Task CreatePeriodicReminder_WithChannelName_ParsesChannelCorrectly()
		{
			...
			repo.Setup(r => r.AddAsync(It.IsAny<Reminder>())).Callback<Reminder>((r) =>
			{
				reminder = r;
			});
			...
			var interaction = new ChatInteractionBuilder(discordMock)
				...
				.OnMessageContaining("How long between each reminder?").RespondWith("2 hours")
				.OnMessageContaining("When should the first reminder be?").RespondWith("2020-01-01 00:00:00")
				.OnMessageContaining("Who should I remind?").RespondWith("Veinlash")
				.OnMessageContaining("What should the reminder say?").RespondWith("This is a test reminder.")
				.OnMessageContaining("What channel should the reminder be sent to?").RespondWith("<#123>")
				.OnMessageContaining("OK, I'll remind Veinlash").Finish()
				.Build();
